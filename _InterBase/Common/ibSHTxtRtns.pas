...
uses
 {$IFDEF MSWINDOWS}
  Windows,SysUtils,Classes,(* BT StrUtil *) ibSHStrUtil,
...
type
  ...
  protected
    ...
    function   IsCOMMENT(Position:integer):boolean;            
  public
...
begin
 ...
 begin
  ...
  begin
   ...
   Inc(pBrOut);
   if (pBrOut>l) and (cBrOut<cBrIn) then
    Exit;
  end;
...
begin
  ...
  begin
   OldTxt:=Text;
   Old:=ibSHTxtRtns.OrderStringTxt(OldTxt, StartPos,EndPos); (*BT*)
   if StartPos>0 then
...
begin
 ...
 if IsDDL(FFirstPos)  or IsCOMMENT(FFirstPos) then  Result:=skDDL
 else
...
function TSQLParser.IsCOMMENT(Position: integer): boolean;
begin
 Result:=   ((Position=1) or (SQLText[Position-1] in CharsBeforeClause))and
         (Len-Position>=6)  and
         (SQLText[Position]   in ['C','c'])   and
         (SQLText[Position+1] in ['O','o'])   and
         (SQLText[Position+2] in ['M','m'])   and
         (SQLText[Position+3] in ['M','m'])   and
         (SQLText[Position+4] in ['E','e'])   and
         (SQLText[Position+5] in ['N','n'])   and
         (SQLText[Position+6] in ['T','t']);
