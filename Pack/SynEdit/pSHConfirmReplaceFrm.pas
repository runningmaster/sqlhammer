...
type
  TpSHConfirmReplaceForm = class(TForm)
    btnReplace: TButton;
...
var
  pSHConfirmReplaceForm: TpSHConfirmReplaceForm;

implementation

uses pSHConsts, pSHCommon;

{$R *.DFM}

{ TpSHConfirmReplaceForm }

procedure TpSHConfirmReplaceForm.FormCreate(Sender: TObject);
begin
  AntiLargeFont(Self);
  Image1.Picture.Icon.Handle := LoadIcon(0, IDI_QUESTION);
end;

procedure TpSHConfirmReplaceForm.FormDestroy(Sender: TObject);
begin
  pSHConfirmReplaceForm := nil;
end;

procedure TpSHConfirmReplaceForm.PrepareShow(AEditorRect: TRect;
