...
{                                                       }
{*******************************************************}
//package  SQLHammer_Common_Package.bpl
...
function PosCh(aCh:Char; const s:string):integer;{$IFDEF D9+} inline;{$ENDIF}
function PosCh1(aCh:Char; const s:string; StartPos:integer):integer; {$IFDEF D9+} inline;{$ENDIF}
...
function  IsBeginPartStr(const PartStr,TargetStr:string):boolean;
...
begin
 ...
 if p=0 then
  ...
  Result:=FastCopy(Str,p+1,MaxInt)
end;
...
begin
 Result:=StrLen(Str)-1;
 while (Result>=0) and (Str[Result]<=' ') do
  Dec(Result);
 Inc(Result);
end;
...
begin
  ...
  while p>0 do
  begin
   if   ((p=1) or (Str[p-1] in BegSub)) and ((Str[p+l1] in EndSub) or (p+l1=l))
   then
   begin
    Result:=p;    Exit;
   end;
...
procedure CopyChars(const Src:string; var  Dest:string;
 StartInSrc,StartInDest,Count:integer );
begin
 Move(Src[StartInSrc],Dest[StartInDest],Count);
end;
...
begin
 if Ansi then
  ...
  Result:=InternalReplaceStr(S,FastUpperCase(S), FastUpperCase(Srch), Replace)
end;
...
var
  S:string;
begin
  S :=ReplaceCIStr(FastCopy(Source,BeginPos,EndPos-BeginPos+1),  Srch, Replace);
  Result := FastCopy(Source,1,BeginPos-1)+s+FastCopy(Source,EndPos+1,MaxInt);
end;
...
var
  S:string;
begin
  S:=ReplaceStr(FastCopy(Source,BeginPos,EndPos-BeginPos+1), Srch, Replace);
  Result := FastCopy(Source,1,BeginPos-1)+s+FastCopy(Source,EndPos+1,MaxInt);
end;
...
begin
  if (LastPos=0) or (LastPos>Length(S)) then
   LastPos := Length(S);
  while LastPos >= FirstPos do
  begin
    if (S[LastPos] >= 'a') and (S[LastPos] <= 'z') then
     S[LastPos]:=Chr(Byte(S[LastPos])-32);
    Dec(LastPos);
  end;
end;
    ...
    begin
      ...
      begin
       ...
       if (FastCopy(FLine,1,p1-1)<>FastCopy(Mask,1,p1-1)) then
         ...
         Result := WildCompare1(FastCopy(FLine,p1,l1),   FastCopy(Mask,p1,l2));
       Exit;
      end;
      ...
      if (Mask[1]=MaskChars[0]) then
       for k := 1 to l1 do
        ...
        begin
         Result :=  WildCompare1(FastCopy(FLine,k,l1-k+1), FastCopy(Mask,2,l2-1));
         if Result then Exit
        end;
      ...
      then
           Result := WildCompare1(FastCopy(FLine,2,l1-1),   FastCopy(Mask,2,l2-1));
    end;
...
begin
 ...
 begin
  ...
  for i :=0  to l do
  begin
   Result:=(Length(SL[i])=0) or (Length(FastTrim(SL[i]))=0);
   if not Result then
    Exit;
  end;
 end;
...
function InternalNeedQuote(const Name:string; Easy:boolean):boolean;{$IFDEF D9+} inline;{$ENDIF}
var
     I,L:integer;
     Ch1,Ch2:Char;
begin
 ...
 begin
  Result:=
   ...
   ((i=1) or not (Name[i] in NonQuotedChars));
