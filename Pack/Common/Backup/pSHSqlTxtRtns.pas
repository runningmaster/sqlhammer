...
uses
 {$IFDEF MSWINDOWS}
  Windows,SysUtils,Classes,(* BT StrUtil *) pSHStrUtil,
...
const
 ...
 procedure AllSQLTables(SQLText:string;FTables:TStrings; WithSP:boolean=False);
 procedure AllTables(const SQLText:string;FTables:Tstrings; WithSP:boolean =False);
...
begin
 Result:=PosExtCI('ORDER',SQLText,CharsBeforeClause,CharsAfterClause,False);
end;
...
begin
 ...
 begin
  AllTables(SQLText,FTables,WithSP);
  ...
  begin
   ...
   FTables[i]:=FastCopy(FTables[i],1,j);
  end;
 end;
end;
...
begin
 ...
 FromText:=ReplaceStr(FastTrim(FromText),#13#10,'  ');
 if FromText='' then
  Exit;
 if PosClause('JOIN',FromText) >0 then
  FromText:=RemoveJoins(FromText);
 if PosCh('(',FromText)>0 then
   FromText:=RemoveSP(FromText,WithSP);
...
begin
 ...
 begin
  s:= TableByAlias(SQLText,FastCopy(aFieldName,1,p-1));
  if (Length(s)=0) then
   Result:=aFieldName
  else
  if (Length(s)>0) and (s[1]<>'"') then
   Result:=s+FastCopy(aFieldName,p,MaxInt)
  else
   Result:=FastCopy(s,2,Length(s)-2)+FastCopy(aFieldName,p,MaxInt)
 end;
end;
...
begin
 ...
 if p=0 then Exit;
// Delete(NSQL,1,p+5);
 NSQL:=FastCopy(NSQL,p+6,MaxInt);
 p:=PosClause('BY',NSQL);
 if p=0 then
  Exit;
// Delete(NSQL,1,p+2);
 NSQL:=FastCopy(NSQL,p+3,MaxInt);
 wc:=WordCount(NSQL,[',']);
 if wc<1 then Exit;
   ...
   case SQLKind of
    ...
    begin
     ...
     if FResultPos>0 then
      Result:=FastCopy(SQLText,FFirstPos+7,FResultPos-8-FFirstPos)
     else
      Result:=FastCopy(SQLText,FFirstPos+7,MaxInt)
    end;
    skInsert:
    begin
     ...
     begin
       ...
       begin
         ...
         '(':
          begin
             Result:=FastCopy(SQLText,FResultPos+5,p-6-FResultPos);
             Exit;
          end;
         'S','s':
          if (Len-p)>=6 then
...
begin
  ...
  begin
    ...
    Result:=
      FastCopy(SQLText,1,p)+BeginWhere+CLRF+NewClause+#13#10+FastCopy(SQLText,p+1,MaxInt)
  end
end;
...
begin
  ...
  case SQLText[Position] of
    ...
    'G', 'g': Result:=
              (Len-Position>=5)  and
              (SQLText[Position+1] in ['R','r'])   and
              (SQLText[Position+2] in ['A','a'])   and
              (SQLText[Position+3] in ['N','n'])   and
              (SQLText[Position+4] in ['T','t'])   and
              (SQLText[Position+5] in CharsAfterClause);
    'R', 'r': Result:=
              (Len-Position>=8)  and
    ...
    'S', 's': if
             ...
             (SQLText[Position+1] in ['E','e'])   and
             (SQLText[Position+2] in ['T','t'])   and
             (SQLText[Position+3] in CharsAfterClause) then
            begin
             vPosition := Position+4;
             while (vPosition <= Len) and (SQLText[vPosition] in CharsBeforeClause) do Inc(vPosition);
             Result:=
              (Len-vPosition>=9)  and
...
begin
 ...
 begin
   case SQLKind of    //
     ...
     case SQLKind of
      skUpdate: FTables.Text:=FastCopy(ns,9,MaxInt);
      skInsert: FTables.Text:=FastCopy(ns,14,MaxInt);
      skDelete: FTables.Text:=FastCopy(ns,14,MaxInt);
     end;
     ...
     begin
       ...
       if p>0 then
        FTables[i]:=FastCopy(FTables[i],1,p);
     end;
   end;
 end;
 Result:=FTables
